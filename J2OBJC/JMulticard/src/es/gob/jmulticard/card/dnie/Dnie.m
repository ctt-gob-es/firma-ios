...
#line 922
      [((JavaMathBigInteger *) nil_chk([((id<JavaSecurityInterfacesRSAPublicKey>) nil_chk(((id<JavaSecurityInterfacesRSAPublicKey>) cast_check([((JavaSecurityCertX509Certificate *) nil_chk([((id<JavaUtilMap>) nil_chk(self->certs_)) getWithId:alias])) getPublicKey], JavaSecurityInterfacesRSAPublicKey_class_())))) getModulus])) bitLength])];
    }
  }
...
void EsGobJmulticardCardDnieDnie_hideCertsWithoutKey(EsGobJmulticardCardDnieDnie *self) {
  for (NSString * __strong alias in nil_chk(self->aliases_)) {
    if ([((id<JavaUtilMap>) nil_chk(self->keyReferences_)) getWithId:alias] == nil) {
      [self->aliases_ removeWithId:alias];
    }
  }
}
